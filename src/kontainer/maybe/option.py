...
class Option(Maybe[ValueT, None], Generic[ValueT]):
    ...
    @override
    def map_other(self, func: Callable[[], AnotherT]) -> Option[ValueT | AnotherT]:
        if self._has_value():
    ...
    @override
    def map_others(
        self, other: ElementT, func: Callable[[ElementT], AnotherT]
    ...
    @override
    def bind_other(
        self, func: Callable[[], Option[AnotherT]]
    ) -> Option[ValueT | AnotherT]:
        if self._has_value():
            return Option(self._val)
    ...
    @override
    def bind_others(
        self, other: ElementT, func: Callable[[ElementT], Option[AnotherT]]
    ) -> Option[ValueT | AnotherT]:
        if self._has_value():
            return Option(self._val)
